            ...
            @forelse ($carts as $cart)
                <div class="flex w-full border-b-2 border-b-gray-300 p-8 gap-8">
                    <div class="basis-64 w-64 bg-gray-200 rounded relative aspect-square overflow-hidden">
                        <img src="{{ url($cart->product->image) }}" alt=""
                            class="absolute w-full h-full object-cover">
                    </div>
                    <div class="grow w-full flex flex-col justify-between">
                        <div class="w-full flex items-center justify-between">
                            <div class="flex flex-col items-start gap-1">
                                <span class="capitalize">{{ $cart->product->name }}</span>
                                <span class="capitalize text-gray-400">product gategory</span>
                                <p class="text-sm font-medium text-gray-900">${{ $cart->product->price }}</p>
                            </div>
                            <input type="number" name="quantity" min="0" max="0"
                                class="bg-gray-100 px-2 py-1 outline-none rounded-md text-center" value="1"
                                {{-- value="{{ $cart->product->quantity }}" --}}
                                readonly />
                            <form action="{{ route('cart.destroy', ['cart' => $cart]) }}" method="post">
                                @csrf
                                @method('delete')
                                <button type="submit" class="w-5 flex flex-col gap-2 relative items-center justify-center cursor-pointer">
